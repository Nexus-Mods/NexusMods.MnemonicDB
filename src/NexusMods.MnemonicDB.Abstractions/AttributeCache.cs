...
public sealed class AttributeCache
{
    ...
    public void Reset(IDb db)
    {
        ...
        foreach (var datom in symbols)
        {
            ...
            var symbol = Symbol.InternPreSanitized((string)datom.V);
        ...
        foreach (var datom in isIndexed)
        {
            ...
            var flags = datom.Prefix.ValueTag switch
            {
                ...
                ValueTag.UInt8 => (IndexedFlags)datom.V,
                _ => IndexedFlags.None
            };
        ...
        {
            var id = datom.E.Value;
            newIsCardinalityMany[(int)id] = (Cardinality)(byte)datom.V == Cardinality.Many;
        }
