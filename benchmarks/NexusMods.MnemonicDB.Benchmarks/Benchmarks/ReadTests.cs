...
public class ReadTests : ABenchmark
{
    ...
    public async Task Setup()
    {
        ...
        _preLoaded = _entityIds.Select(id => File.Load(_db, id)).ToArray();
    }
    ...
    {
        ulong sum = 0;
        sum += File.Load(_db, _readId).Size.Value;
        return sum;
    }
    ...
    public long ReadAll()
    {
        return _entityIds.Select(id => File.Load(_db, id))
